...
## Feature selection.
## ======================= ##
## We do not exclude any variables so far.
...
set.seed(123)
train_index <- sample(1:nrow(data), size = 0.70 * nrow(data))

Train_raw <- data[train_index, ]
Test_raw  <- data[-train_index, ]

#==== Step 1: Learn Transformations ONLY from the Training Data ================#

features_for_clustering <- c("total_sulfur_dioxide", "chlorides", "volatile_acidity")
train_for_clustering <- Train_raw[, features_for_clustering]

# Learn scaling parameters (mean, sd) from the training data for these features
scaling_values_cluster <- list(
  mean = apply(train_for_clustering, 2, mean),
  sd = apply(train_for_clustering, 2, sd)
)

scaled_train_cluster <- scale(train_for_clustering,
                              center = scaling_values_cluster$mean,
                              scale = scaling_values_cluster$sd)

kmeans_model <- kmeans(scaled_train_cluster, centers = 2, nstart = 25)
cluster_summary <- aggregate(train_for_clustering,
                             by = list(cluster = kmeans_model$cluster),
                             FUN = mean)
white_wine_cluster_id <- cluster_summary$cluster[which.max(cluster_summary$total_sulfur_dioxide)]


predictor_names <- setdiff(names(Train_raw), "quality")
train_predictors_raw <- Train_raw[, predictor_names]

# Learn scaling parameters (mean, sd) for ALL predictors from the training data
scaling_values_all <- list(
  mean = apply(train_predictors_raw, 2, mean, na.rm = TRUE),
  sd = apply(train_predictors_raw, 2, sd, na.rm = TRUE)
)


#==== Step 2: Define Corrected Preprocessing Function =========================#

preprocess_data <- function(df, kmeans_model, white_wine_id,
                            cluster_scaling, all_predictor_scaling) {
  
  features_for_clustering <- c("total_sulfur_dioxide", "chlorides", "volatile_acidity")
  df_for_clustering <- df[, features_for_clustering]
  
  scaled_df_cluster <- scale(df_for_clustering,
                             center = cluster_scaling$mean,
                             scale = cluster_scaling$sd)
  
  centers <- kmeans_model$centers
  cluster_preds <- FNN::knnx.index(data = centers, query = scaled_df_cluster, k = 1)
  
  df$is_white <- as.factor(ifelse(cluster_preds == white_wine_id, 1, 0))
  
  predictor_names_to_scale <- names(all_predictor_scaling$mean)
  df_predictors_to_scale <- df[, predictor_names_to_scale]
  
  df_predictors_scaled <- scale(df_predictors_to_scale,
                                center = all_predictor_scaling$mean,
                                scale = all_predictor_scaling$sd)
  
  final_df <- cbind(as.data.frame(df_predictors_scaled),
                    is_white = df$is_white,
                    quality = df$quality)
...
Train <- preprocess_data(Train_raw, kmeans_model, white_wine_cluster_id,
                         scaling_values_cluster, scaling_values_all)
Test  <- preprocess_data(Test_raw, kmeans_model, white_wine_cluster_id,
                         scaling_values_cluster, scaling_values_all)
...
{
## Standardization after data splitting to avoid data leakage.
...
tryCatch({
  ...
  for (i in 1:nrow(param_grid)) {
    ...
    current_min_node_size <- param_grid$min.node.size[i]
    fold_f1_scores <- c()
    ...
    for (k in 1:nfolds) {
      ...
      rf_model <- ranger(
        ...
        data = train_fold, 
        mtry = current_mtry,
        min.node.size = current_min_node_size,
        num.trees = 500
      )
      
      val_preds_obj <- predict(rf_model, data = val_fold)
      val_preds <- val_preds_obj$predictions
  ...
  print(rf_results[order(rf_results$F1_Score, decreasing = TRUE), ])
  
  best_params_row <- rf_results[which.max(rf_results$F1_Score), ]
  best_mtry <- best_params_row$mtry
  best_min_node_size <- best_params_row$min.node.size
  ...
  rf_test_preds_obj <- predict(final_rf_model, data = Test)
  rf_test_preds <- rf_test_preds_obj$predictions
  ...
  tryCatch({
    ...
    plot_importance <- ggplot(importance_data, aes(x = reorder(Variable, Importance), y = Importance)) +
      ...
      coord_flip() +
      labs(title = "Feature Importance (Ranger Classification)",
           subtitle = paste("Best mtry =", best_mtry, "& Best min.node.size =", best_min_node_size),
           x = "Features",
    ...
    Path <- file.path(Charts_Directory, "04_Ranger_FeatureImportance_Class.png")
    ggsave(filename = Path, plot = plot_importance, width = 3750, height = 1833, units = "px", dpi = 300)
    ...
    plot_cm_raw <- ggplot(cm_data_raw, aes(x = Actual, y = Predicted, fill = Freq)) +
      ...
      labs(title = "Ranger Confusion Matrix (Raw Counts)", x = "Actual Quality", y = "Predicted Quality", fill = "Frequency") +
      theme_light() +
      theme(plot.title = element_text(hjust = 0.5))
    
    Path <- file.path(Charts_Directory, "04b_Ranger_ConfusionMatrix.png")
    ggsave(filename = Path, plot = plot_cm_raw, width = 3750, height = 1833, units = "px", dpi = 300)
    
    cm_percent_rf <- cm_data_raw %>% group_by(Actual) %>% mutate(Percentage = Freq / sum(Freq), Label = paste0(round(Percentage * 100), "%"))
    plot_cm_normalized <- ggplot(cm_percent_rf, aes(x = Actual, y = Predicted, fill = Percentage)) +
      ...
      labs(title = "Normalized Confusion Matrix (Ranger)", subtitle = "Rows sum to 100%", x = "Actual Quality", y = "Predicted Quality", fill = "Percentage of Actual") +
      theme_light() +
      theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), aspect.ratio = 1)
    
    Path <- file.path(Charts_Directory, "04e_Ranger_Normalized_CM.png")
    ggsave(filename = Path, plot = plot_cm_normalized, width = 2500, height = 2500, units = "px", dpi = 300)
    
    results_df <- Test
    results_df$Predicted <- rf_test_preds
    results_df$Status <- ifelse(results_df$quality == results_df$Predicted, "Correct", "Incorrect")
    misclassified_df <- results_df %>% filter(Status == "Incorrect")
    ...
    plot_error_analysis <- ggplot(misclassified_df, aes(x = quality, y = Predicted)) +
      ...
      labs(title = "Analysis of Misclassifications (Ranger)", subtitle = "Where are the model's errors concentrated?", x = "Actual Quality", y = "Predicted Quality", size = "Number of Errors", color = "Count") +
      theme_light() +
      theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), aspect.ratio = 1) +
      guides(color = "none")
    
    Path <- file.path(Charts_Directory, "04f_Ranger_Error_Analysis.png")
    ggsave(filename = Path, plot = plot_error_analysis, width = 2500, height = 2500, units = "px", dpi = 300)
